...
class FeatureDropoutFunction(torch.autograd.function.InplaceFunction):
    ...
    def forward(cls, ctx, input, batch_idxs, p=0.5, train=False, inplace=False):
        ...
        if ctx.p > 0 and ctx.train:
            ...
            ctx.noise = ctx.noise[batch_idxs.batch_idxs_torch.long(), :]
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, hparams, n_head, d_model, d_k, d_v, residual_dropout=0.1, attention_dropout=0.1, d_positional=None):
        ...
        self.attention = ScaledAttention(
            hparams, attention_dropout=attention_dropout)
...
class MultiLevelEmbedding(nn.Module):
    ...
    def forward(self, xs, pre_words_idxs, batch_idxs, extra_content_annotations=None):
        ...
        timing_signal = torch.cat(
            [self.position_table[:seq_len, :] for seq_len in batch_idxs.seq_lens_np], dim=0)
    ...
    def parse_batch(self, sentences, golds=None):
        ...
        for snum, sentence in enumerate(sentences):
            ...
            for (tag, word) in [(START, START)] + sentence + [(STOP, STOP)]:
                tag_idxs[i] = 0 if not self.use_tags else self.tag_vocab.index_or_unk(
                    tag, TAG_UNK)
        ...
        if self.encoder is not None:
            ...
            fencepost_annotations_start = features.masked_select(all_word_start_mask.to(
                torch.uint8).unsqueeze(-1)).reshape(-1, features.shape[-1])
        ...
        with torch.no_grad():
            for i, (start, end) in enumerate(zip(fp_startpoints, fp_endpoints)):
                if hasattr(golds[i], 'oracle_label') == False or hasattr(golds[i], 'nocache') == False:
                    continue
                # remove
                try:
        ...
        cells_label_scores = self.f_label(
            fencepost_annotations_end[cells_j.long()] - fencepost_annotations_start[cells_i.long()])
        cells_label_scores = torch.cat([
            cells_label_scores.new_zeros((cells_label_scores.size(0), 1)),
            cells_label_scores
        ], 1)
        cells_label = cells_label.type(torch.int64)
        ...
        for snum, (start, end) in enumerate(zip(fp_startpoints, fp_endpoints)):
            ...
            #Window has a numpy is that long type on Window 
            
            loss = loss + 0.5 * self.loss_func(arc_score[1:, :], from_numpy(np.array(arc_gather)).requires_grad_(False).long()) \
                + 0.5 * self.loss_funt(type_score[1:, :], from_numpy(
                    np.array(type_gather)).requires_grad_(False).long())
