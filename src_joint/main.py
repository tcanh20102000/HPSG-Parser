...
def run_train(args, hparams):
    ...
    load_path = None
    #load_path = 'models/run26th/joint_single_best_dev=79.29_devuas=87.27_devlas=79.63.pt'
    ...
    best_model_path = "{}_best_dev={:.2f}_devuas={:.2f}_devlas={:.2f}".format(
        args.model_path_base, best_fscore, best_uas, best_las)
    print("Saved best model is {}.pt in epoch {}.".format(
        best_model_path, best_epoch_num))
...
def run_test(args):
    ...
    with open('predicted_result.csv', 'w', encoding='utf-8') as f:
        header = ['Stt', 'Word', 'Predicted head',
                  'Predicted type', 'True head', 'True type']
        writer = csv.writer(f, delimiter='\t')
        writer.writerow(header)
        for i in range(0, len(test_pred_head)):
            stt = list(range(1, len(test_pred_head[i]) + 1))
            writer.writerows(zip(
                stt, dep_test_word[i], test_pred_head[i], test_pred_type[i], dep_test_headid[i], dep_test_type[i]))
            writer.writerows('\n')
