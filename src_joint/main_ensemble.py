...
def run_train(args, hparams):
    ...
    # delete later
    with open('data/VnData/VnData3/dep/dep_types.txt', 'w') as f:
        for i in range(0, len(dep_types)):
            f.write(str(dep_types[i]) + '\n')
    with open('data/VnData/VnData3/dep/dep_heads.txt', 'w') as f:
        for i in range(0, len(dep_heads)):
            f.write(str(dep_heads[i]) + '\n')
    #
    ...
    for i, tree in enumerate(train_parse):
        ...
        if len(const_sentences) != len(dep_sentences[i]):
            continue
        nodes = [tree]
        while nodes:
            node = nodes.pop()
    ...
    # load path: put path to best trained model
    load_path = None
    load_path = 'models/run5th/joint_single_base_dev=78.54_devuas=76.07_devlas=69.78.pt'
    if load_path is not None:
        print(f"Loading parameters from {load_path}")
        info = torch_load(load_path)
        parser = Zparser.ChartParser.from_spec(
    ...
    best_model_path = 'models/joint_model'
...
def get_predicted(parser, args, test_treebank):
    parser.eval()
    test_predicted = []
...
def get_ensemble_result(pred_head1, pred_type1, pred_head2, pred_type2, pred_head3, pred_type3):
    #model 3 has weight 1.5, model 2 has weight 1.4, model 1 has weight 1.
    final_test_pred_head = []
    final_test_pred_type = []
    
    END = len(pred_head1)
    for i in range(0, END):
        sentence_head_1 = pred_head1[i]
        sentence_type_1 = pred_type1[i]
        sentence_head_2 = pred_head2[i]
        sentence_type_2 = pred_type2[i]
        sentence_head_3 = pred_head3[i]
        sentence_type_3 = pred_type3[i]
        
        if(sentence_head_1 == sentence_head_2):
            final_test_pred_head.append(sentence_head_2)
            final_test_pred_type.append(sentence_type_2)
            continue
        
        #voting on sentence head on the side of parser 3
        final_test_pred_head.append(sentence_head_3)
        final_test_pred_type.append(sentence_type_3)
            
    return final_test_pred_head, final_test_pred_type
...
def run_test(args):
    ...
    parser2 = ''
    parser3 = ''
    if(args.model_path_base_nd and args.model_path_base_rd):
        ensemble_mode = True
        print("Loading model from {}...".format(args.model_path_base_nd))

        info = torch_load(args.model_path_base_nd)
        assert 'hparams' in info['spec'], "Older savefiles not supported"
        parser2 = Zparser.ChartParser.from_spec(info['spec'], info['state_dict'])
        parser2.eval()
        
        print("Loading model from {}...".format(args.model_path_base_rd))
        info = torch_load(args.model_path_base_rd)
        assert 'hparams' in info['spec'], "Older savefiles not supported"
        parser3 = Zparser.ChartParser.from_spec(info['spec'], info['state_dict'])
        parser3.eval()
    ...
    parser.eval()
    test_predicted = []
    test_predicted = get_predicted(parser, args, test_treebank)
    ...
    print(test_pred_head)
    print(test_pred_type)
    ...
    if(ensemble_mode):
        ...
        test_predicted_2 = get_predicted(parser2, args, test_treebank)
        test_predicted_3 = get_predicted(parser3, args, test_treebank)
        
        test_pred_head2 = [[leaf.father for leaf in tree.leaves()]
                      for tree in test_predicted_2]
        test_pred_type2 = [[leaf.type for leaf in tree.leaves()]
                      for tree in test_predicted_2]
        
        test_pred_head3 = [[leaf.father for leaf in tree.leaves()]
                      for tree in test_predicted_3]
        test_pred_type3 = [[leaf.type for leaf in tree.leaves()]
                      for tree in test_predicted_3]
        print('Parser 2')         
        print(test_pred_head2)
        print(test_pred_type2)
        print('Parser3')
        print(test_pred_head3)
        print(test_pred_type3)
        
        final_test_pred_head, final_test_pred_type = get_ensemble_result(test_pred_head,test_pred_type,test_pred_head2, test_pred_type2, test_pred_head3, test_pred_type3)
        print('Ensemble')
        print(final_test_pred_head)
        print(final_test_pred_type)
        
    

    stats, stats_nopunc, stats_root, test_total_inst = dep_eval.eval(len(test_pred_head), dep_test_word, dep_test_pos,
                                                                     final_test_pred_head,
                                                                     final_test_pred_type, dep_test_headid, dep_test_type,
...
def main():
    ...
    subparser.add_argument("--dep-train-ptb-path",
                           default="data/ptb_train_3.3.0.sd")
    subparser.add_argument("--dep-dev-ptb-path",
                           default="data/ptb_dev_3.3.0.sd")
    ...
    subparser.add_argument("--model-path-base", required=True)
    subparser.add_argument("--model-path-base-nd", required=False)
    subparser.add_argument("--model-path-base-rd", required=False)
    subparser.add_argument("--evalb-dir", default="EVALB/")
    subparser.add_argument(
    ...
    subparser.add_argument("--deptest-ptb-path",
                           default="data/ptb_test_3.3.0.sd")
